export const MOVIE_URL = 'http://localhost:8383/api/movie'
export const CREW_URL = 'http://localhost:8383/api/crew'
export const SEAT_URL = 'http://localhost:8383/api/seatPattern'
export const IMAGE_URL = 'http://localhost:8383/api/image'
export const CINEMA_URL = 'http://localhost:8383/api/cinema'
export const THEATER_URL = 'http://localhost:8383/api/theater'
export const SHOW_TIME_URL = 'http://localhost:8383/api/showtime'
export const TICKET_URL = 'http://localhost:8383/api/ticket'
export const BOOK_SEAT_URL = 'http://localhost:8383/api/bookSeat'
export const COUPON_URL = 'http://localhost:8383/api/coupon'
export const PAYMENT_URL = 'http://localhost:8383/api/userpayment'
export const USER_URL = 'http://localhost:8383/api/user'
export const OTP_URL = 'http://localhost:8383/api/otp'
